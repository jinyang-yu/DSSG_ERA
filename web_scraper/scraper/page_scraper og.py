...
class PageScraper:
  ...
  def extract_clean_text(self, html: str) -> Tuple[str, str]:
    ...
    # Heuristically find the main article container
    allowed_classes = {
    "article__body", "ArticlePage-articleBody", "main-article", "content-body",
    ...
    article_container = (
        ...
        next(
            (
                tag for tag in soup.find_all("div", class_=lambda c: c and any(allowed in cls for allowed in allowed_classes for cls in c))
            ), 
            None
        )
    )
  ...
  #         filtered_links = self.matcher.batch_match(all_links, [])
  #         for link, title in filtered_links.items():
  ...
  #           if not soup or not check_valid_article(link, soup, title):
  #             continue
